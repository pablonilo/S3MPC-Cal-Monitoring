function Monitoring_CAL_S3A( folderin )
%Monitoring_CAL: Routine phase S3A monitoring report
% calling: Monitoring_CAL_S3A('C:\Users\Pablo\Desktop\S3MPC\data\CAL\SR_1_CAL\CYCLE_ALL\');
...
calfolders=dir([folderin 'S3A_SR_1_CAL____*']);
calmatfiles=dir([folderin 'cal_ncread_S3A_SR_1_CAL____*.mat']);
...
elseif ~isempty(calmatfiles) && ~isempty(calfolders) % matlab L1 files and nc folders --> find matches
    ...
    for i_folder=1:size(L1bread,1)
        if isdir([folderin L1bread(i_folder,:)]) && strcmp('S3A_SR_1_CAL____20',L1bread(i_folder,1:18)) % only folders that begin as CAL L1b S3
...
dirmat=dir([folderin 'cal_ncread_S3A_SR_1_CAL____*.mat']); % resulting collection of matlab files
...
if recnum>0
    save([folderin 'S3MPC_S3A_CAL1_LRM_IQ_data_' timemeas_name],'lrmiq');   % save the data into a mat file
    ...
    namef=['S3MPC_S3A_CAL1_LRM_IQ_Calibration_Areas_'  timemeas_name];
    figure
    title(['S3A SRAL CAL1 LRM Calibration Areas from '  datestr(lrmiq.time(1),1) ' to ' datestr(lrmiq.time(end),1)  '.'],'fontsize', 14);
    axesm eckert4;
    framem; gridm;
    axis off
    ...
    % Ku
    namef=['S3MPC_S3A_CAL1_LRM_Ku_TimeDelay_'  timemeas_name];
    ...
    title(['S3A SRAL CAL1 LRM Ku Time Delay. Trend from '  datestr(lrmiq.time(1),1) ' to ' datestr(lrmiq.time(end),1) '.']);
    ...
    % C
    namef=['S3MPC_S3A_CAL1_LRM_C_TimeDelay_'  timemeas_name];
    ...
    title(['S3A SRAL CAL1 LRM C Time Delay. Trend from '  datestr(lrmiq.time(1),1) ' to ' datestr(lrmiq.time(end),1) '.']);
    xlabel('Date'); ylabel('Time Delay [m]');
    legend('show');
    if (Xaxisrange(end)-Xaxisrange(1))*365.25 < 90
        datetick('x',20,'keeplimits','keepticks') % mmmyydd for short time series
    ...
    namef=['S3MPC_S3A_CAL1_LRM_Ku_ALLDelays_'  timemeas_name];
    Xaxisrange  = lrmiq.time(lrmiq.ptrdelku_f==0)/365.25;
    ...
    title(['S3A SRAL CAL1 LRM Ku Delays. Trends from '  datestr(lrmiq.time(1),1) ' to ' datestr(lrmiq.time(end),1) '.']);
    xlabel('Date'); ylabel('Time Delay [m]');
    legend('show');
    if (Xaxisrange(end)-Xaxisrange(1))*365.25 < 90
        datetick('x',20,'keeplimits','keepticks') % mmmyydd for short time series
    ...
    namef=['S3MPC_S3A_CAL1_LRM_C_ALLDelays_'  timemeas_name];
    Xaxisrange  = lrmiq.time(lrmiq.ptrdelc_f==0)/365.25;
    ...
    title(['S3A SRAL CAL1 LRM C Delays. Trends from '  datestr(lrmiq.time(1),1) ' to ' datestr(lrmiq.time(end),1) '.']);
    xlabel('Date'); ylabel('Time Delay [m]');
    legend('show');
    if (Xaxisrange(end)-Xaxisrange(1))*365.25 < 90
        datetick('x',20,'keeplimits','keepticks') % mmmyydd for short time series
    ...
    % Ku
    namef=['S3MPC_S3A_CAL1_LRM_Ku_Power_' timemeas_name];
    ...
    title(['S3A SRAL CAL1 LRM Ku Integrated & Max Power. Trends from ' datestr(lrmiq.time(1),1) ' to ' datestr(lrmiq.time(end),1) '.']);
    xlabel('Date');
    legend('show');
    if (Xaxisrange(end)-Xaxisrange(1))*365.25<90
    ...
    % C
    namef=['S3MPC_S3A_CAL1_LRM_C_Power_' timemeas_name];
    ...
    set(H1,'linewidth',3,'marker', 'o','markersize',6,'DisplayName',['Integrated Power. Mean= ' num2str(meanv0) ' dB. Slope=' num2str(p0(1)) ' dB/yr. Stdev=' num2str(stdev0) ' dB']);
    set(H2,'linewidth',3,'marker', 'o','markersize',6,'DisplayName',['Maximum Power.  Mean= ' num2str(meanv1) ' dB. Slope=' num2str(p1(1)) ' dB/yr. Stdev=' num2str(stdev1) ' dB']);
    title(['S3A SRAL CAL1 LRM C Integrated & Max Power. Trends from ' datestr(lrmiq.time(1),1) ' to ' datestr(lrmiq.time(end),1) '.']);
    xlabel('Date');
    legend('show');
    ...
    % Ku
    freq2dist=cst.light_speed/2*chd.SRAL_PTR_FFT_Features.FFT_Step_Time_Ku/chd.SRAL_PTR_FFT_Features.FFT_Step_Freq_Ku; % Hz to m
    namef=['S3MPC_S3A_CAL1_LRM_Ku_Width_'  timemeas_name];
    ...
    title(['S3A SRAL CAL1 LRM Ku PTR Width. Trend from '  datestr(lrmiq.time(1),1) ' to ' datestr(lrmiq.time(end),1) '.']);
    ...
    % C
    freq2dist=cst.light_speed/2*chd.SRAL_PTR_FFT_Features.FFT_Step_Time_C/chd.SRAL_PTR_FFT_Features.FFT_Step_Freq_C; % Hz to m
    namef=['S3MPC_S3A_CAL1_LRM_C_Width_'  timemeas_name];
    ...
    title(['S3A SRAL CAL1 LRM C PTR Width. Trend from '  datestr(Xaxisrange(1)*365.25,1) ' to ' datestr(Xaxisrange(end)*365.25,1) '.']);
    xlabel('Date'); ylabel('PTR Width [m]');
    legend('show');
    if (Xaxisrange(end)-Xaxisrange(1))*365.25 < 90
        datetick('x',20,'keeplimits','keepticks') % mmmyydd for short time series
    ...
    % Ku distribution
    freq2dist=cst.light_speed/2*chd.SRAL_PTR_FFT_Features.FFT_Step_Time_Ku/chd.SRAL_PTR_FFT_Features.FFT_Step_Freq_Ku; % Hz to m
    namef=['S3MPC_S3A_CAL1_LRM_Ku_Sec_Lobes_Distrib'  timemeas_name];
    figure; hold all;
    title(['S3A SRAL CAL1 LRM Ku Secondary Lobes Distribution. From '  datestr(lrmiq.time(1),1) ' to ' datestr(lrmiq.time(end),1) '.']);
    for i_rec=1:recnum
    ...
    % C distribution
    freq2dist=cst.light_speed/2*chd.SRAL_PTR_FFT_Features.FFT_Step_Time_C/chd.SRAL_PTR_FFT_Features.FFT_Step_Freq_C; % Hz to m
    namef=['S3MPC_S3A_CAL1_LRM_C_Sec_Lobes_Distrib'  timemeas_name];
    figure; hold all;
    title(['S3A SRAL CAL1 LRM C Secondary Lobes Distribution. From '  datestr(lrmiq.time(1),1) ' to ' datestr(lrmiq.time(end),1) '.']);
    for i_rec=1:recnum
    ...
    % Ku Power Trend
    namef=['S3MPC_S3A_CAL1_LRM_Ku_Sec_Lobes_Power_Trend'  timemeas_name];
    figure; hold all;
    title(['S3A SRAL CAL1 LRM Ku Secondary Lobes Power Trend from '  datestr(lrmiq.time(1),1) ' to ' datestr(lrmiq.time(end),1) '.']);
    ...
    % C Power Trend
    namef=['S3MPC_S3A_CAL1_LRM_C_Sec_Lobes_Power_Trend'  timemeas_name];
    figure; hold all;
    title(['S3A SRAL CAL1 LRM C Secondary Lobes Power Trend from '  datestr(lrmiq.time(1),1) ' to ' datestr(lrmiq.time(end),1) '.']);
    ...
    namef=['S3MPC_S3A_CAL1_LRM_Sec_Lobes_Power_std_slope'  timemeas_name];
    figure; hold all;
    title(['S3A SRAL CAL1 LRM Secondary Lobes Power Stdev & Inter-Annual Slope from '  datestr(lrmiq.time(1),1) ' to ' datestr(lrmiq.time(end),1) '.']);
...
if recnum>0
    save([folderin 'S3MPC_S3A_CAL1_SAR_NORM_data_' timemeas_name],'sar');   % save the data into a mat file
    ...
    namef=['S3MPC_S3A_CAL1_SAR_Calibration_Areas_'  timemeas_name];
    figure
    title(['S3A SRAL CAL1 SAR Calibration Areas from '  datestr(sartime(1),1) ' to ' datestr(sartime(end),1) '.'],'fontsize', 14);
    axesm eckert4;
    framem; gridm;
    axis off
    geoshow('landareas.shp', 'FaceColor', 'black');
    ...
    % Ku
    namef=['S3MPC_S3A_CAL1_SAR_Ku_TimeDelay_'  timemeas_name];
    ...
    title(['S3A SRAL CAL1 SAR Ku Time Delay. Trend from '  datestr(sartime(1),1) ' to ' datestr(sartime(end),1) '.']);
    xlabel('Date'); ylabel('Time Delay [m]');
    legend('show');
    if (Xaxisrange(end)-Xaxisrange(1))*365.25 < 90
        datetick('x',20,'keeplimits','keepticks') % mmmyydd for short time series
    ...
    % C
    namef=['S3MPC_S3A_CAL1_SAR_C_TimeDelay_'  timemeas_name];
    ...
    title(['S3A SRAL CAL1 SAR C Time Delay. Trend from '  datestr(sar.time(1),1) ' to ' datestr(sar.time(end),1) '.']);
    xlabel('Date'); ylabel('Time Delay [m]');
    legend('show');
    if (Xaxisrange(end)-Xaxisrange(1))*365.25<90
        datetick('x',20,'keeplimits','keepticks') % mmmyydd for short time series
    ...
    % Ku
    freq2dist=cst.light_speed/2*chd.SRAL_PTR_FFT_Features.FFT_Step_Time_Ku/chd.SRAL_PTR_FFT_Features.FFT_Step_Freq_Ku; % Hz to m
    namef=['S3MPC_S3A_CAL1_SAR_Ku_ALLDelays_'  timemeas_name];
    Xaxisrange  = sar.time(sar.ptrdelku_f==0)/365.25;
    ...
    title(['S3A SRAL CAL1 SAR Ku Delays. Trends from '  datestr(sar.time(1),1) ' to ' datestr(sar.time(end),1) '.']);
    xlabel('Date'); ylabel('Time Delay [m]');
    legend('show');
    if (Xaxisrange(end)-Xaxisrange(1))*365.25<90
        datetick('x',20,'keeplimits','keepticks') % mmmyydd for short time series
    ...
    % C
    freq2dist=cst.light_speed/2*chd.SRAL_PTR_FFT_Features.FFT_Step_Time_C/chd.SRAL_PTR_FFT_Features.FFT_Step_Freq_C; % Hz to m
    namef=['S3MPC_S3A_CAL1_SAR_C_ALLDelays_'  timemeas_name];
    Xaxisrange  = sar.time(sar.ptrdelc_f==0)/365.25;
    ...
    title(['S3A SRAL CAL1 SAR C Delays. Trends from '  datestr(sar.time(1),1) ' to ' datestr(sar.time(end),1) '.']);
    xlabel('Date'); ylabel('Time Delay [m]');
    legend('show');
    if (Xaxisrange(end)-Xaxisrange(1))*365.25<90
        datetick('x',20,'keeplimits','keepticks') % mmmyydd for short time series
    ...
    % Ku
    namef=['S3MPC_S3A_CAL1_SAR_Ku_Power_' timemeas_name];
    ...
    set(H1,'linewidth',3,'marker', 'o','markersize',6,'DisplayName',['Integrated Power. Mean=' num2str(meanv0) ' dB. Slope=' num2str(p0(1)) ' dB/yr. Stdev=' num2str(stdev0) ' dB']);
    set(H2,'linewidth',3,'marker', 'o','markersize',6,'DisplayName',['Maximum Power. Mean=' num2str(meanv1) ' dB. Slope=' num2str(p1(1)) ' dB/yr. Stdev=' num2str(stdev1) ' dB']);
    title(['S3A SRAL CAL1 SAR Ku Integrated & Max Power. Trends from ' datestr(sar.time(1),1) ' to ' datestr(sar.time(end),1) '.']);
    xlabel('Date');
    legend('show');
    set(AX(2),'XTickLabel',[]); % keep only one X axis
    if (Xaxisrange(end)-Xaxisrange(1))*365.25<90
    ...
    namef=['S3MPC_S3A_CAL1_SAR_Ku_Power_modelfitting' timemeas_name];
    Xaxisrange = sar.time(sar.ptrpowku_f==0)/365.25;
    Y1 = sar.ptrpowku(sar.ptrpowku_f==0);
    ...
    title(['S3A SRAL CAL1 SAR Ku Integrated Power. Fitting by Power Model. Drop of 3.5dB at ' num2str(floor(time4ice)) '/' num2str(round((time4ice-floor(time4ice))*12)) '.']);
    ...
    % C
    namef=['S3MPC_S3A_CAL1_SAR_C_Power_' timemeas_name];
    ...
    set(H1,'linewidth',3,'marker', 'o','markersize',6,'DisplayName',['Integrated Power. Mean=' num2str(meanv0) ' dB. Slope=' num2str(p0(1)) ' dB/yr. Stdev=' num2str(stdev0) ' dB']);
    set(H2,'linewidth',3,'marker', 'o','markersize',6,'DisplayName',['Maximum Power. Mean=' num2str(meanv1) ' dB. Slope=' num2str(p1(1)) ' dB/yr. Stdev=' num2str(stdev1) ' dB']);
    title(['S3A SRAL CAL1 SAR C Integrated & Max Power. Trends from ' datestr(sar.time(1),1) ' to ' datestr(sar.time(end),1) '.']);
    xlabel('Date');
    legend('show');
    set(AX(2),'XTickLabel',[]); % keep only one X axis
    if (Xaxisrange(end)-Xaxisrange(1))*365.25<90
    ...
    namef=['S3MPC_S3A_CAL1_SAR_Ku_Width_'  timemeas_name];
    ...
    title(['S3A SRAL CAL1 SAR Ku PTR Width. Trend from '  datestr(sar.time(1),1) ' to ' datestr(sar.time(end),1) '.']);
    xlabel('Date'); ylabel('PTR Width [m]');
    legend('show');
    if (Xaxisrange(end)-Xaxisrange(1))*365.25<90
        datetick('x',20,'keeplimits','keepticks') % mmmyydd for short time series
    ...
    namef=['S3MPC_S3A_CAL1_SAR_C_Width_'  timemeas_name];
    ...
    title(['S3A SRAL CAL1 SAR C PTR Width. Trend from '  datestr(sar.time(1),1) ' to ' datestr(sar.time(end),1) '.']);
    xlabel('Date'); ylabel('PTR Width [m]');
    legend('show');
    if (Xaxisrange(end)-Xaxisrange(1))*365.25<90
        datetick('x',20,'keeplimits','keepticks') % mmmyydd for short time series
    ...
    % Ku distribution
    freq2dist=cst.light_speed/2*chd.SRAL_PTR_FFT_Features.FFT_Step_Time_Ku/chd.SRAL_PTR_FFT_Features.FFT_Step_Freq_Ku; % Hz to m
    namef=['S3MPC_S3A_CAL1_SAR_Ku_Sec_Lobes_Distrib'  timemeas_name];
    figure; hold all;
    title(['S3A SRAL CAL1 SAR Ku Secondary Lobes Distribution. From '  datestr(sartime(1),1) ' to ' datestr(sartime(end),1) '.']);
    for i_rec=1:recnum
    ...
    % C distribution
    freq2dist=cst.light_speed/2*chd.SRAL_PTR_FFT_Features.FFT_Step_Time_C/chd.SRAL_PTR_FFT_Features.FFT_Step_Freq_C; % Hz to m
    namef=['S3MPC_S3A_CAL1_SAR_C_Sec_Lobes_Distrib'  timemeas_name];
    figure; hold all;
    title(['S3A SRAL CAL1 SAR C Secondary Lobes Distribution. From '  datestr(sartime(1),1) ' to ' datestr(sartime(end),1) '.']);
    for i_rec=1:recnum
    ...
    % Ku Power Trend
    namef=['S3MPC_S3A_CAL1_SAR_Ku_Sec_Lobes_Power_Trend'  timemeas_name];
    figure; hold all;
    title(['S3A SRAL CAL1 SAR Ku Secondary Lobes Power Trend from '  datestr(sartime(1),1) ' to ' datestr(sartime(end),1) '.']);
    seclobepow_annualslope_ku(1:20)=0;
    seclobepow_std_ku(1:20)=0;
    for i_lobe=1:20
        if i_lobe<11
    ...
    % C Power Trend
    namef=['S3MPC_S3A_CAL1_SAR_C_Sec_Lobes_Power_Trend'  timemeas_name];
    figure; hold all;
    title(['S3A SRAL CAL1 SAR C Secondary Lobes Power Trend from '  datestr(sartime(1),1) ' to ' datestr(sartime(end),1) '.']);
    seclobepow_annualslope_c(1:20)=0;
    seclobepow_std_c(1:20)=0;
    for i_lobe=1:20
        if i_lobe<11
    ...
    % Ku & C sec lobes POWER std & slope
    namef=['S3MPC_S3A_CAL1_SAR_Sec_Lobes_Power_std_slope'  timemeas_name];
    figure; hold all;
    title(['S3A SRAL CAL1 SAR Secondary Lobes Power Stdev & Inter-Annual Slope from '  datestr(sartime(1),1) ' to ' datestr(sartime(end),1) '.']);
    plot([(-10:-1) (1:10)], seclobepow_annualslope_ku, 'o-','linewidth',3,'markersize',6,'DisplayName','seclobe annual slope Ku');
    plot([(-10:-1) (1:10)], seclobepow_std_ku*100, 'o-','linewidth',3,'markersize',6,'DisplayName','seclobe std Ku');
    plot([(-10:-1) (1:10)], seclobepow_annualslope_c, 'o-','linewidth',3,'markersize',6,'DisplayName','seclobe annual slope C');
    plot([(-10:-1) (1:10)], seclobepow_std_c*100, 'o-','linewidth',3,'markersize',6,'DisplayName','seclobe std C');
    ...
    namef=['S3MPC_S3A_CAL1_SAR_Burst_Power_2D_'  timemeas_name];
    ...
    title(['S3A SRAL CAL1 SAR Ku Burst Power [dB]. From '  datestr(sar.time(1),1) ' to ' datestr(sar.time(end),1) '.']);
    ...
    namef=['S3MPC_S3A_CAL1_SAR_Burst_Phase_2D_'  timemeas_name];
    ...
    title(['S3A SRAL CAL1 SAR Ku Burst Phase [rad]. From '  datestr(sar.time(1),1) ' to ' datestr(sar.time(end),1) '.']);
    xlabel('Pulse number in the Burst'); ylabel('Date');
    datetick('y',20,'keeplimits','keepticks') % mmmyydd for short time series
    colormap jet;
    colorbar
    ...
    namef=['S3MPC_S3A_CAL1_SAR_Burst_Power_'  timemeas_name];
    indexburstpower=~(max(sar.burstpower,[],2)==0 & min(sar.burstpower,[],2)==0);
    ...
    title(['S3A SRAL CAL1 SAR Ku Burst Power. Averaged from '  datestr(sar.time(1),1) ' to ' datestr(sar.time(end),1) '.']);
    ...
    namef=['S3MPC_S3A_CAL1_SAR_Burst_Phase_'  timemeas_name];
    indexburstphase=~(max(sar.burstphase,[],2)==0 & min(sar.burstphase,[],2)==0);
    ...
    title(['S3A SRAL CAL1 SAR Ku Burst Phase. Averaged from '  datestr(sar.time(1),1) ' to ' datestr(sar.time(end),1) '.']);
    ...
    namef=['S3MPC_S3A_CAL1_SAR_Burst_Arrays_std_' timemeas_name];
    figure;
    ...
    title(['S3A SRAL CAL1 SAR Burst Power & Phase pulse by pulse standard deviations from ' datestr(sartime(1),1) ' to ' datestr(sartime(end),1) '.']);
    ...
    namef=['S3MPC_S3A_CAL1_SAR_Burst_Arrays_slopes_' timemeas_name];
    figure;
    ...
    title(['S3A SRAL CAL1 SAR Burst Power & Phase arrays slopes from ' datestr(sartime(1),1) ' to ' datestr(sartime(end),1) '.']);
    xlabel('Date');
    legend('show');
    if (Xaxisrange(end)-Xaxisrange(1))*365.25<90
        datetick(AX(1),'x',20,'keeplimits','keepticks') % mmmyydd for short time series
...
if recnum>0
    save([folderin 'S3MPC_S3A_CAL1_SAR_Auto_data_' timemeas_name],'sarauto');   % save the data into a mat file
    ...
    namef=['S3MPC_S3A_CAL1_SAR_Auto_Calibration_Areas_'  timemeas_name];
    figure
    title(['S3A SRAL CAL1 Auto-Calibration Areas from '  datestr(sarautotime(1),1) ' to ' datestr(sarautotime(end),1)  '.'],'fontsize', 14);
    axesm eckert4;
    framem; gridm;
    axis off
    geoshow('landareas.shp', 'FaceColor', 'black');
    ...
    namef=['S3MPC_S3A_CAL1_SAR_AutoCal_corr&ref_Ku&C_'  timemeas_name];
    figure; hold all;
    ...
    title(['S3A SRAL AutoCAL Averaged (Corrected - Reference). From '  datestr(sarautotime(1),1) ' to ' datestr(sarautotime(end),1) '.']);
    legend('show');
    xlim([0 65]);
    saveas(gcf,[folderin namef],'jpg');
    saveas(gcf,[folderin namef],'fig');
    ...
    namef=['S3MPC_S3A_CAL1_SAR_AutoCal_corr_progression_Ku_'  timemeas_name];
    figure; imagesc(sarauto.time,1:63,sarauto_prog_ku);
    title(['S3A SRAL AutoCAL Ku-band Attenuation progression [dB]. From '  datestr(sarauto.time(1),1) ' to ' datestr(sarauto.time(end),1) '.']);
    ...
    namef=['S3MPC_S3A_CAL1_SAR_AutoCal_corr_progression_C_'  timemeas_name];
    figure; imagesc(sarauto.time,1:63,sarauto_prog_c);
    title(['S3A SRAL AutoCAL C-band Attenuation progression [dB]. From '  datestr(sarauto.time(1),1) ' to ' datestr(sarauto.time(end),1) '.']);
    ylabel('Attenuation Step [0:63]'); xlabel('Date');
    colormap bone;
    colorbar
    if (sarauto.time(end)-sarauto.time(1))<90
...
save([folderin 'S3MPC_S3A_CAL2_SAR_Ku_data_' timemeas_name],'cal2sarku');   % save the data into a mat file
...
if recnum>0
    save([folderin 'S3MPC_S3A_CAL2_SAR_C_data_' timemeas_name],'cal2sarc');   % save the data into a mat file
    ...
    namef=['S3MPC_S3A_CAL2_SAR_Calibration_Areas_'  timemeas_name];
    figure
    title(['S3A SRAL CAL2 SAR Calibration Areas from '  datestr(cal2kutime(1),1) ' to ' datestr(cal2kutime(end),1)  '.'],'fontsize', 14);
    axesm eckert4;
    framem; gridm;
    axis off
    geoshow('landareas.shp', 'FaceColor', 'black');
    ...
    % Ku waveforms Trend
    namef=['S3MPC_S3A_CAL2_SAR_Ku_WFMs'  timemeas_name];
    figure;
    surf((1:128),cal2sarku.time',cal2sarku.meas);
    xlabel('Range bin'); ylabel('Date'); zlabel('GPRW Power. [FFT p.u.]');
    title(['S3A SRAL CAL2 SAR Ku GPRW Waveforms. From '  datestr(cal2kutime(1),1) ' to ' datestr(cal2kutime(end),1) '.']);
    ...
    % C waveforms Trend
    namef=['S3MPC_S3A_CAL2_SAR_C_WFMs'  timemeas_name];
    figure;
    surf((1:128),cal2sarc.time',cal2sarc.meas);
    xlabel('Range bin'); ylabel('Date'); zlabel('GPRW Power. [FFT p.u.]');
    title(['S3A SRAL CAL2 SAR C GPRW Waveforms. From '  datestr(cal2ctime(1),1) ' to ' datestr(cal2ctime(end),1) '.']);
    ...
    namef=['S3MPC_S3A_CAL2_SAR_Ku_2DWFMsripples'  timemeas_name];
    figure;
    imagesc((11:118),cal2sarku.time',cal2sarku.meas(:,11:118));
    xlabel('Range bin'); ylabel('Date'); zlabel('GPRW Power. [FFT p.u.]');
    title(['S3A SRAL CAL2 SAR Ku GPRW Waveforms Ripples (samples 11 to 118). From '  datestr(cal2kutime(1),1) ' to ' datestr(cal2kutime(end),1) '.']);
    if (cal2sarku.time(end)-cal2sarku.time(1))<90
        datetick('y',20,'keeplimits','keepticks') % mmmyydd for short time series
        ylim([sar.time(1)-1 sar.time(end)+1]);
    else
    ...
    namef=['S3MPC_S3A_CAL2_SAR_C_2DWFMsripples'  timemeas_name];
    figure;
    imagesc((11:118),cal2sarc.time',cal2sarc.meas(:,11:118));
    xlabel('Range bin'); ylabel('Date'); zlabel('GPRW Power. [FFT p.u.]');
    title(['S3A SRAL CAL2 SAR C GPRW Waveforms Ripples (samples 11 to 118). From '  datestr(cal2ctime(1),1) ' to ' datestr(cal2ctime(end),1) '.']);
    if (cal2sarc.time(end)-cal2sarc.time(1))<90
        datetick('y',20,'keeplimits','keepticks') % mmmyydd for short time series
        ylim([sar.time(1)-1 sar.time(end)+1]);
    else
    ...
    namef=['S3MPC_S3A_CAL2_SAR_Ku&C_AvgWfm'  timemeas_name];
    figure; hold all;
    title(['S3A SRAL CAL2 SAR Ku (blue) and C (green) GPRW averaged wfm. From '  datestr(cal2kutime(1),1) ' to ' datestr(cal2kutime(end),1) '.']);
    ...
    % Ku Right & Left Mean
    namef=['S3MPC_S3A_CAL2_SAR_Ku_Mean'  timemeas_name];
    ...
    figure; hold all;
    title(['S3A SRAL CAL2 SAR Ku. Right (blue) and Left (red) GPRW wfm sides Mean. From '  datestr(cal2kutime(1),1) ' to ' datestr(cal2kutime(end),1) '.']);
    ...
    % C Right & Left Mean
    namef=['S3MPC_S3A_CAL2_SAR_C_Mean'  timemeas_name];
    Xaxisrangeright=cal2sarku.time(cal2sarku.flag_mean_right==1)/365.25;
    Xaxisrangeleft=cal2sarku.time(cal2sarku.flag_mean_left==1)/365.25;
    ...
    figure; hold all;
    title(['S3A SRAL CAL2 SAR C. Right (blue) and Left (red) GPRW wfm sides Mean. From '  datestr(cal2ctime(1),1) ' to ' datestr(cal2ctime(end),1) '.']);
    ...
    % Ku Right & Left stdev
    namef=['S3MPC_S3A_CAL2_SAR_Ku_Stdev'  timemeas_name];
    ...
    figure; hold all;
    title(['S3A SRAL CAL2 SAR Ku. Right (blue) and Left (red) GPRW wfm sides Stdev. From '  datestr(cal2kutime(1),1) ' to ' datestr(cal2kutime(end),1) '.']);
    ...
    % C Right & Left stdev
    namef=['S3MPC_S3A_CAL2_SAR_C_Stdev'  timemeas_name];
    ...
    figure; hold all;
    title(['S3A SRAL CAL2 SAR C. Right (blue) and Left (red) GPRW wfm sides Stdev. From '  datestr(cal2kutime(1),1) ' to ' datestr(cal2kutime(end),1) '.']);
    ...
    % Ku Right & Left stdev
    namef=['S3MPC_S3A_CAL2_SAR_Ku_Peak2peak'  timemeas_name];
    ...
    figure; hold all;
    title(['S3A SRAL CAL2 SAR Ku. Right (blue) and Left (red) GPRW wfm sides Peak to Peak. From '  datestr(cal2kutime(1),1) ' to ' datestr(cal2kutime(end),1) '.']);
    ...
    % C Right & Left stdev
    namef=['S3MPC_S3A_CAL2_SAR_C_Peak2peak'  timemeas_name];
    ...
    figure; hold all;
    title(['S3A SRAL CAL2 SAR C. Right (blue) and Left (red) GPRW wfm sides Peak to Peak. From '  datestr(cal2kutime(1),1) ' to ' datestr(cal2kutime(end),1) '.']);
    ...
    % Ku Right & Left Slope
    namef=['S3MPC_S3A_CAL2_SAR_Ku_Slope'  timemeas_name];
    ...
    figure; hold all;
    title(['S3A SRAL CAL2 SAR Ku. Right (blue) and Left (red) GPRW wfm sides linear regression Slopes. From '  datestr(cal2kutime(1),1) ' to ' datestr(cal2kutime(end),1) '.']);
    ...
    % C Right & Left Slope
    namef=['S3MPC_S3A_CAL2_SAR_C_Slope'  timemeas_name];
    ...
    figure; hold all;
    title(['S3A SRAL CAL2 SAR C. Right (blue) and Left (red) GPRW wfm sides linear regression Slopes. From '  datestr(cal2ctime(1),1) ' to ' datestr(cal2ctime(end),1) '.']);
    ...
    namef=['S3MPC_S3A_CAL2_SAR_Ku_DeslopedStdev'  timemeas_name];
    ...
    figure; hold all;
    title(['S3A SRAL CAL2 SAR Ku. Right (blue) and Left (red) GPRW wfm sides desloped Stdev. From '  datestr(cal2kutime(1),1) ' to ' datestr(cal2kutime(end),1) '.']);
    ...
    namef=['S3MPC_S3A_CAL2_SAR_C_DeslopedStdev'  timemeas_name];
    ...
    figure; hold all;
    title(['S3A SRAL CAL2 SAR C. Right (blue) and Left (red) GPRW wfm sides desloped Stdev. From '  datestr(cal2ctime(1),1) ' to ' datestr(cal2ctime(end),1) '.']);
    ...
    % Ku & C Max location
    namef=['S3MPC_S3A_CAL2_SAR_Ku_MaxLocation'  timemeas_name];
    figure; hold all;
    title(['S3A SRAL CAL2 SAR Ku (blue) and C (green) GPRW wfm Maximum Location. From '  datestr(cal2kutime(1),1) ' to ' datestr(cal2kutime(end),1) '.']);
...
xlswrite('SRALreporting.xlsx', {'Reporting Table'}, tabxls, 'A1');
xlswrite('SRALreporting.xlsx', {['SRAL Calibration data period: ' timemeas_name '.']}, tabxls, 'A2');
xlswrite('SRALreporting.xlsx', {'mean', 'annual slope', 'standard deviation', 'mean', 'annual slope', 'standard deviation'}, tabxls, 'C4');
xlswrite('SRALreporting.xlsx', {['BOM to C' cycle_number]}, tabxls, 'A12');
...
xlswrite('SRALreporting.xlsx', {'LRM CAL1 Time Delay',mean(lrmiq.ptrdelku(lrmiq.ptrdelku_f==0)),p_lrmdelku(1)*1000,std(lrmiq.ptrdelku(lrmiq.ptrdelku_f==0))*1000,mean(lrmiq.ptrdelc(lrmiq.ptrdelc_f==0)),p_lrmdelc(1)*1000,std(lrmiq.ptrdelc(lrmiq.ptrdelc_f==0))*1000}, tabxls, 'B5');
xlswrite('SRALreporting.xlsx', {'LRM CAL1 Power',mean(lrmiq.ptrpowku(lrmiq.ptrpowku_f==0)),p_lrmpowku(1),std(lrmiq.ptrpowku(lrmiq.ptrpowku_f==0)),mean(lrmiq.ptrpowc(lrmiq.ptrpowc_f==0)),p_lrmpowc(1),std(lrmiq.ptrpowc(lrmiq.ptrpowc_f==0))}, tabxls, 'B7');
xlswrite('SRALreporting.xlsx', {'LRM CAL1 PTR Width',mean(lrmiq.ptrwidthku(lrmiq.ptrwidthku_f==0))*freq2distku,p_lrmwidku(1)*1000,std(lrmiq.ptrwidthku(lrmiq.ptrwidthku_f==0)*1000*freq2distku),mean(lrmiq.ptrwidthc(lrmiq.ptrwidthc_f==0))*freq2distc,p_lrmwidc(1)*1000,std(lrmiq.ptrwidthc(lrmiq.ptrwidthc_f==0)*1000*freq2distc)}, tabxls, 'B9');
...
xlswrite('SRALreporting.xlsx', {'SAR CAL1 Time Delay',mean(sar.ptrdelku(sar.ptrdelku_f==0)),p_sardelku(1)*1000,std(sar.ptrdelku(sar.ptrdelku_f==0))*1000,mean(sar.ptrdelc(sar.ptrdelc_f==0)),p_sardelc(1)*1000,std(sar.ptrdelc(sar.ptrdelc_f==0))*1000}, tabxls, 'B6');
xlswrite('SRALreporting.xlsx', {'SAR CAL1 Power',mean(sar.ptrpowku(sar.ptrpowku_f==0)),p_sarpowku(1),std(sar.ptrpowku(sar.ptrpowku_f==0)),mean(sar.ptrpowc(sar.ptrpowc_f==0)),p_sarpowc(1),std(sar.ptrpowc(sar.ptrpowc_f==0))}, tabxls, 'B8');
xlswrite('SRALreporting.xlsx', {'SAR CAL1 PTR Width',mean(sar.ptrwidthku(sar.ptrwidthku_f==0))*freq2distku,p_sarwidku(1)*1000,std(sar.ptrwidthku(sar.ptrwidthku_f==0)*1000*freq2distku),mean(sar.ptrwidthc(sar.ptrwidthc_f==0))*freq2distc,p_sarwidc(1)*1000,std(sar.ptrwidthc(sar.ptrwidthc_f==0)*1000*freq2distc)}, tabxls, 'B10');
